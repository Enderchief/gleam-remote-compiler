...
import type { LanguageInfo } from './register';
...
export class SimpleLanguageInfoProvider {
  ...
  private getTokensProviderForLanguage(
    ...
    const encodedLanguageId = languages.getEncodedLanguageId(language);
...
function createStyleElementForColorsCSS(): HTMLStyleElement {
  ...
  let previous = document.querySelector('injected');
  if (previous) {
    previous.replaceWith(style);
    return style;
  }
  if (monacoColors) {
